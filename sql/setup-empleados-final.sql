-- =================================================================
-- SETUP EMPLEADOS FINAL — Sal y Canela
-- Archivo único, idempotente. Corre todo lo necesario para que
-- el admin pueda crear empleados y que estos puedan iniciar sesión.
-- Ejecutar en Supabase → SQL Editor
-- =================================================================

-- ── 1. Columna usu_activo (si no existe) ─────────────────────────
ALTER TABLE usuarios
  ADD COLUMN IF NOT EXISTS usu_activo BOOLEAN NOT NULL DEFAULT true;

-- ── 2. verificar_login actualizado (bloquea inactivos) ───────────
...
BEGIN
  SELECT usu_activo INTO v_activo FROM usuarios WHERE usu_usuario = p_usuario;
  IF NOT FOUND THEN
...
-- ── 4. Tabla y secuencia de empleados ────────────────────────────
...
DROP POLICY IF EXISTS "empleados_anon_all" ON empleados;
CREATE POLICY "empleados_anon_all" ON empleados FOR ALL TO anon USING (true) WITH CHECK (true);

-- ── 5. crear_empleado (con fix email NULL) ────────────────────────
...
BEGIN
  ...
  INSERT INTO usuarios(usu_usuario, usu_email, usu_nombre, usu_apellido, usu_telefono, usu_password, usu_activo)
  VALUES (
    TRIM(p_usuario),
    v_email,
    TRIM(p_nombre),
  ...
  INSERT INTO empleados(usu_id, emp_cargo, emp_fecha_ingreso, emp_activo, emp_observaciones)
  VALUES (
    v_usu_id,
    COALESCE(NULLIF(TRIM(p_cargo), ''), 'Empleado'),
...
BEGIN
  ...
  UPDATE empleados
  SET emp_cargo = COALESCE(NULLIF(TRIM(p_cargo), ''), 'Empleado'),
      emp_observaciones = COALESCE(TRIM(p_observaciones), '')
...
-- ── 8. cambiar_estado_usuario (activa/desactiva en ambas tablas) ──
...
-- ── 9. listar_usuarios — solo clientes (excluye empleados) ────────
